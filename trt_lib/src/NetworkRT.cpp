...
NetworkRT::NetworkRT(Network *net, const char *name) {
    ...
    if(!fileExist(name)) {
        ...
        configRT->setMaxWorkspaceSize(1 << 40);
...
#endif
        if(engineRT == nullptr) {
            std::cerr << "try to setMaxWorkspaceSize(1 << 50) or bigger, maybe can solver this problem.\n";
...
dnnType* NetworkRT::infer(dataDim_t &dim, dnnType* data) {
    ...
    checkCuda(cudaMemcpyAsync(buffersRT[buf_input_idx], data, batches * input_dim.tot() * sizeof(dnnType),
                              cudaMemcpyDeviceToDevice, stream))

    contextRT->enqueue(batches, buffersRT, stream, nullptr);
    checkCuda(cudaMemcpyAsync(output, buffersRT[buf_output_idx], batches * output_dim.tot() * sizeof(dnnType),
                              cudaMemcpyDeviceToDevice, stream));
