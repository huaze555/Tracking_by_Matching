#ifndef TRACKER_BY_MATCHER_HPP
#define TRACKER_BY_MATCHER_HPP
...
///
    struct CV_EXPORTS TrackedObject {
        ...
        TrackedObject(const cv::Rect &rect, float confidence, int frame_idx, int object_id)
                : rect(rect), confidence(confidence), frame_idx(frame_idx), object_id(object_id), timestamp(0) {}
    };
...
///
    ...
    class TrackerByMatcher{
    public:
        TrackerByMatcher(float fps, const std::vector<int>& desired_class_id = { -1 });

        TrackedObjects track(const cv::Mat& frame, const std::vector<DetectBox>& origin_bboxes,
